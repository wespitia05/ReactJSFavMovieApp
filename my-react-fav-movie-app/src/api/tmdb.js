...
async function getMoviesByYear(year, mediaType = "movie", sortBy = "popularity.desc", startPage = 1) {
    let allMovies = [];
    let totalResults = 0;
    let totalPages = 1;
    ...
    for (let page = startPage; page <= startPage + 3; page++) {
        const yearParam = mediaType === "tv" ? `first_air_date_year=${year}` : `primary_release_year=${year}`;
        const url = `${base_url}/discover/${mediaType}?${yearParam}&include_adult=false&language=en-US&page=${page}&sort_by=${sortBy}&vote_count.gte=50&api_key=${api}`;
...
async function getMoviesByCountry(countryCode, mediaType = "movie", sortBy = "popularity.desc", startPage = 1) {
    let allMovies = [];
    let totalResults = 0;
    let totalPages = 1;
...
async function getMoviesByGenre(genreId, mediaType = "movie", sortBy = "popularity.desc", startPage = 1) {
    ...
    for (let page = startPage; page <= startPage + 3; page++) {
        ...
        if (!res.ok) {
            // if tmdb returns 401, 404, etc, we throw an error
            throw new Error("TMDB movies by genre failed")
        };
...
async function getMoviesByTheme(keywordId, mediaType = "movie", sortBy = "popularity.desc", startPage = 1) {
    ...
    for (let page = startPage; page <= startPage + 3; page++) {
        ...
        if (!res.ok) {
            // if tmdb returns 401, 404, etc, we throw an error
            throw new Error("TMDB movies by theme failed")
        };
...
export {searchMulti, getMovieDetails, getImages, getActorDetails, getPersonCredits, 
        ...
        getMoviesByPrimaryLanguage, getMoviesByGenre, getMoviesByTheme}